...
public class Snake {
	...
	public void update() {
		Location nexthead = null;
		
		//1. use a switch statement to check on the currentDirection
		//   of the snake and calculate its next x and y position.
		switch(currentDirection) {
			case UP:
				nexthead = new Location(head.getLocation().x,head.getLocation().y - 1);
				break;
			case LEFT:
				nexthead = new Location(head.getLocation().x - 1,head.getLocation().y);
				break;
			case RIGHT:
				nexthead = new Location(head.getLocation().x + 1,head.getLocation().y);
				break;
	...
	public boolean isOutOfBounds() {
		...
		if (head.getLocation().x < 0 || head.getLocation().x > _00_SnakeGame.WIDTH) {
			System.out.println("x = " + head.getLocation().x);
			return true;
		} else if (head.getLocation().y < 0 || head.getLocation().y > _00_SnakeGame.HEIGHT ) {
			System.out.println("y = " + head.getLocation().y);
			return true;
		}
		return false;
	}
